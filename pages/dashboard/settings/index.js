...
function AltPhoto({ style, seed, ...props }) {
  return (
    <div className={settings.altProfile} {...props}>
      <img
...
function EditProfileModal({ open, handleClose, user }) {
  const { register, handleSubmit, control, watch } = useForm({
    defaultValues: { img: user?.photoURL, name: user?.displayName },
  });
  ...
  const submit = ({ img, name }) => {
    if (img.search("blob:") == 0) {
      uploadFile(`${user?.uid}-${fileRef.files[0].name}`, fileRef.files[0])
        .then((fileMeta) => {
          toast.success("Profile photo upload successfull 🎉");
          getDownloadURL(fileMeta.ref)
            .then((imageURL) => {
              updateProfile(user, { photoURL: imageURL, displayName: name })
                .then(() => {
                  handleClose();
                ...
                .catch((error) => {
                  toast.error(
                    `Unable to Update Profile, error code ${error?.code}`
                  );
                });
            })
            .catch(() => {});
      ...
      updateProfile(user, { photoURL: img, displayName: name })
        .then(() => {
          handleClose();
          toast.success("Successfully Updated Profile");
        })
    ...
    >
      <div className={settings.editProfileRoot}>
              ...
              <div>
                {styles.map((style) => (
                  <AltPhoto
                    ...
                    onClick={() =>
                      onChange(
                        `https://avatars.dicebear.com/api/${style}/${user?.uid}.svg`
                      )
                    }
                  />
                ))}
        ...
        <div className={settings.profileDetails}>
          ...
          <Input
            className={settings.profileDetailsInput}
            {...register("name")}
            hint={"UserName"}
          />
